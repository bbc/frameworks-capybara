...
class CapybaraSetup
  ...
  def register_selenium_driver(opts, remote_opts, custom_opts)
    Capybara.register_driver :selenium do |app|
      ...
      if opts[:browser] == :remote
        ...
        # set sauce specific parameters - will this scupper other on sauce remote jobs?
        add_custom_caps(caps, custom_opts) if remote_opts[:url].include? 'saucelabs'
  ...
  def set_client_proxy(opts)
    # set proxy on client connection if required, note you may use ENV['HTTP_PROXY'] for setting in browser (ff profile) but not for client conection, hence allow for PROXY_ON=false
    Selenium::WebDriver::Proxy.new(http: opts[:http_proxy]) if opts[:http_proxy] && opts[:webdriver_proxy_on] != 'false'
  end
